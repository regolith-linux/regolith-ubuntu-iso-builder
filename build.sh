...
cat "$REPO_ROOT/releases/config.sh.tmpl" | envsubst | sudo tee "$REPO_ROOT/scripts/default_config.sh" >/dev/null
...
YYYYMMDD="$(date +%Y%m%d)"
OUTPUT_DIR="$REPO_ROOT/builds/$ARCH"
mkdir -p "$OUTPUT_DIR"
...
FNAME="regolith-$VERSION-$PARENT_DISTRO_NAME-$PARENT_DISTRO_VERSION$FNAME_PARTIAL-$ARCH"
